...
def get_pending():
    ...
    c = conn.cursor()
    c.execute("""
        SELECT id, university, title, url, detected_at 
    ...
    conn.close()
    return jsonify([{
        "id": r[0], "university": r[1],
        "title": r[2], "url": r[3], "detected_at": r[4]
    } for r in rows])
...
def get_all():
    ...
    return jsonify([{
        ...
        "approved_at": r[6], "approved_by": r[7]
    } for r in rows])
...
def approve():
    ...
    c = conn.cursor()
    c.execute("""
        UPDATE updates SET status='approved',
...
def get_students():
    ...
    c = conn.cursor()
    c.execute("""
        SELECT id, phone, name, university, course, year, registered_at
